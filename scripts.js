...
update = function() {
    ...
    let createDb = "CREATE DATABASE ubhub;";
    query.push(createDb);
    ...
    let userProcedure =
        ...
        "END";
    query.push(userProcedure);

    let locationSimpleProcedure =
    ...
    let indicatorsTable =
        ...
        "`categoryId` INT, " +
        "`archetype` VARCHAR(255), " + // governance and management, public involvement, location profile, natural infrastructure, species and habitat assessments
        "`weight` FLOAT(10, 2), " +
        "`required` BIT, " +
        "`description` VARCHAR(2048) CHARACTER SET utf8, " +
    ...
    let createIndicatorInProgramTable = null;

    let createIndicatorInProgramProcedure =
        ...
        "SELECT * FROM indicators WHERE id=LAST_INSERT_ID();\n" +
        "END";
    query.push(createIndicatorInProgramProcedure);
